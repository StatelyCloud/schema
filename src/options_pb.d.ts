...
export declare type MessageOptions = Message<"stately.schemamodel.MessageOptions"> & {
  ...
  /**
   * Deprecated in favor of KeyPath.index_paths.
   *
   * @generated from field: repeated stately.schemamodel.MessageOptions.Index indexes = 4 [deprecated = true];
   * @deprecated
   */
...
/**
 * @generated from message stately.schemamodel.MessageOptions.IndexConfig
 */
export declare type MessageOptions_IndexConfig =
  Message<"stately.schemamodel.MessageOptions.IndexConfig"> & {
    /**
     * path_template is a url pattern that will be used to generate the path for
     * this index key. For more details see KeyPath.path_template.
     *
     * To indicate that this is a group-local index (LSI), the path should start with
     * a the segment `/@LSI-{1-4}` which identifies which group-local index to use.
     * The remainder of the path is the index's key path (a sort key).
     * Note: these indexes inherit the partition key from the parent key path.
     *
     * To indicate that this is a global secondary index (GSI), the path should start with
     * a the segment `/@GSI-{1-20}` which identifies which global secondary index to use.
     * The following segments are the index's key path (a partition key AND sort key).
     *
     * Valid index numbers are 1-4 for LSIs, and 1-20 for GSIs.
     *
 ...
 */
export declare const MessageOptions_IndexConfigSchema: GenMessage<MessageOptions_IndexConfig>;
...
export declare type MessageOptions_KeyPath =
  Message<"stately.schemamodel.MessageOptions.KeyPath"> & {
    ...
    /**
     * index_configs is a list of index configurations that are persisted with this key path.
     * These are defined per KeyPath because that's how DynamoDB works:
     * We must *store* and item for it to be indexed.
     *
     * @generated from field: repeated stately.schemamodel.MessageOptions.IndexConfig index_configs = 3;
     */
    indexConfigs: MessageOptions_IndexConfig[];
  };
